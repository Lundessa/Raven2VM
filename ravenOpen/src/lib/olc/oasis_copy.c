...
//    { CON_SEDIT,  real_shop,   sedit_save_internally, sedit_setup_existing, "scopy", "shop" },
 //   { CON_TRIGEDIT, real_trigger, trigedit_save,   trigedit_setup_existing, "tcopy", "trigger" },
  ...
  if (!*buf2 || !is_number(buf1) || !is_number(buf2)) {
    sendChar(ch, "Syntax: %s <source vnum> <target vnum>\r\n", oasis_copy_info[i].command);
    return;
  }
  ...
  if (src_rnum == NOWHERE) {
    sendChar(ch, "The source %s (#%d) does not exist.\r\n", oasis_copy_info[i].text, src_vnum);
    return;
  }
  ...
  if (dst_rnum != NOWHERE) {
    sendChar(ch, "The target %s (#%d) already exists.\r\n", oasis_copy_info[i].text, dst_vnum);
    return;
  }
  ...
  for (d = descriptor_list; d; d = d->next) {
	...
	sendChar(ch, "The target %s (#%d) is currently being edited by %s.\r\n",
  ...
  if ((OLC_ZNUM(d) = real_zone_by_thing(dst_vnum)) == NOWHERE) {
    sendChar(ch, "Sorry, there is no zone for the given vnum (#%d)!\r\n", dst_vnum);
    free(d->olc);
    d->olc = NULL;
    return;
  }
...
ACMD(do_dig)
{
  ...
  if (!*sdir || !*sroom) {
    sendChar(ch, "Format: dig <direction> <room> - to create an exit\r\n"
  ...
  if (rvnum == NOTHING) {
    if (W_EXIT(IN_ROOM(ch), dir)) {
      ...
      sendChar(ch, "You remove the exit to the %s.\r\n", dirs[dir]);
      return;
    }
    sendChar(ch, "There is no exit to the %s.\r\n"
                     "No exit removed.\r\n", dirs[dir]);
    return;
  }
  /* Can't dig in a direction, if it's already a door. */
  if (W_EXIT(IN_ROOM(ch), dir)) {
      sendChar(ch, "There already is an exit to the %s.\r\n", dirs[dir]);
      return;
  }
  ...
  if (zone == NOWHERE) {
    sendChar(ch, "You cannot link to a non-existing zone!\r\n");
    return;
  }
  if (!can_edit_zone(ch, zone)) {
    sendChar(ch, "You do not have permission to edit room #%d.\r\n", rvnum);
    return;
  }
  ...
  //  redit_save_internally(d);
    ...
    sendChar(ch, "New room (%d) created.\r\n", rvnum);
    cleanup_olc(d, CLEANUP_ALL);
  ...
  sendChar(ch, "You make an exit %s to room %d (%s).\r\n",
                   dirs[dir], rvnum, world[rrnum].name);

  /* Check if we can dig from there to here. */
  if (W_EXIT(rrnum, rev_dir[dir]))
    sendChar(ch, "You cannot dig from %d to here. The target room already has an exit to the %s.\r\n",
