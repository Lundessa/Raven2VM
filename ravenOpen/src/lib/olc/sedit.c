...
void sMenu(DescriptorData * d);
void sNamelistMenu(DescriptorData *d);
void sTypesMenu(DescriptorData *d);
void sProductsMenu(DescriptorData *d);
void sRoomsMenu(DescriptorData *d);
void sCompactRoomsMenu(DescriptorData *d);
void sShopFlagsMenu(DescriptorData *d);
void sNoTradeMenu(DescriptorData *d);
void sSaveInternally(DescriptorData *d);
void copyShop(struct shop_data *tshop, struct shop_data *fshop);
void copyList(int **tlist, int *flist);
void copyTypeList(struct shop_buy_data **tlist, struct shop_buy_data *flist);
void sAddToTypeList(struct shop_buy_data **list, struct shop_buy_data *new);
void sRemoveFromTypeList(struct shop_buy_data **list, int num);
void freeShopStringss(struct shop_data *shop);
void freeTypeList(struct shop_buy_data **list);
void sFreeShop(struct shop_data *shop);
void sModifyString(char **str, char *new);
...
{ 
  ...
  copyShop(OLC_SHOP(d), shop_index + rshop_num);
  sMenu(d);
}
...
void copyShop(struct shop_data *tshop, struct shop_data *fshop)
{
  ...
  /*. Copy lists over .*/ 
  copyList(&(S_ROOMS(tshop)), S_ROOMS(fshop));
  copyList(&(S_PRODUCTS(tshop)), S_PRODUCTS(fshop));
  copyTypeList(&(tshop->type), fshop->type);

  /*. Copy notification strings over .*/
  freeShopStringss(tshop);
...
void sFreeShop(struct shop_data *shop)
{ freeShopStringss(shop);
  freeTypeList(&(S_NAMELISTS(shop)));
  free(S_ROOMS(shop));
...
{ 
  ...
  {  mudlog(BRF, LVL_BUILDER, TRUE, "OLCERR: OLC: Cannot open shop file!");
     return;
  }
  if (fprintf(shop_file, "CircleMUD v3.0 Shop File~\n") < 0)
  {  mudlog(BRF, LVL_BUILDER, TRUE, "OLCERR: OLC: Cannot write to shop file!");
     return;
  }
...
/*-------------------------------------------------------------------*/
  case SEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      sSaveInternally(d);
      mudlog( CMP, LVL_BUILDER, TRUE, "OLC: %s edits shop %d", GET_NAME(d->character),
               OLC_NUM(d));	      
...
/*-------------------------------------------------------------------*/
  /*. String edits .*/
  case SEDIT_NOITEM1:
    sModifyString(&S_NOITEM1(OLC_SHOP(d)), arg);
    break;
  case SEDIT_NOITEM2:
    sModifyString(&S_NOITEM2(OLC_SHOP(d)), arg);
    break;
  case SEDIT_NOCASH1:
    sModifyString(&S_NOCASH1(OLC_SHOP(d)), arg);
    break;
  case SEDIT_NOCASH2:
    sModifyString(&S_NOCASH2(OLC_SHOP(d)), arg);
    break;
  case SEDIT_NOBUY:
    sModifyString(&S_NOBUY(OLC_SHOP(d)), arg);
    break;
  case SEDIT_BUY:
    sModifyString(&S_BUY(OLC_SHOP(d)), arg);
    break;
  case SEDIT_SELL:
    sModifyString(&S_SELL(OLC_SHOP(d)), arg);
    break;
  case SEDIT_NAMELIST:
    { struct shop_buy_data new_entry;
      ...
      sAddToTypeList(&(S_NAMELISTS(OLC_SHOP(d))), &new_entry);
    }
    sNamelistMenu(d);
    return;
...
/*-------------------------------------------------------------------*/
  ...
  case SEDIT_DELETE_TYPE:
    sRemoveFromTypeList(&(S_NAMELISTS(OLC_SHOP(d))), atoi(arg));
    sNamelistMenu(d);
    return;
  ...
  case SEDIT_SHOP_FLAGS:
    i = MAX(0, MIN(NUM_SHOP_BITS, atoi(arg)));
    if (i>0)
    { /*. Toggle bit .*/
      i = 1 << (i-1);
    ...
    break;
  case SEDIT_NOTRADE:
    i = MAX(0, MIN(NUM_TRADE_LETTERS, atoi(arg)));
    if (i>0)
    { /*. Toggle bit .*/
      i = 1 << (i-1);
...
/*-------------------------------------------------------------------*/
  default:
    ...
    cleanup_olc(d, CLEANUP_ALL);
    mudlog(BRF, LVL_BUILDER, TRUE, "OLCERR: OLC: sParse(): Reached default case!");
    break;
  }
/*-------------------------------------------------------------------*/
/*. END OF CASE 
