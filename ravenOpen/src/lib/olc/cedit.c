...
ACMD(do_oasis_cedit)
{
  ...
  if (GET_LEVEL(ch) < LVL_IMPL) {
    sendChar(ch, "You can't modify the game configuration.\r\n");
    return;
  }
  ...
  if (!*buf1) {
    ...
    sendChar(ch, "Yikes!  Stop that, someone will get hurt!\r\n");
    return;
  }

  sendChar(ch, "Saving the game configuration.\r\n");
...
static void cedit_setup(struct descriptor_data *d)
{
  ...
  OLC_CONFIG(d)->play.plague_is_contagious = CONFIG_PLAGUE_IS_CONTAGIOUS;
  OLC_CONFIG(d)->play.quest_active         = CONFIG_QUEST_ACTIVE;
  /* Crash Saves */
...
static void cedit_save_internally(struct descriptor_data *d)
{
  ...
  CONFIG_PLAGUE_IS_CONTAGIOUS = OLC_CONFIG(d)->play.plague_is_contagious;
  CONFIG_QUEST_ACTIVE         = OLC_CONFIG(d)->play.quest_active;
...
int save_config( IDXTYPE nowhere )
{
  ...
  fprintf(fl, "* Are plagues contagious?\n"
              "plague = %d\n\n", CONFIG_PLAGUE_IS_CONTAGIOUS);
  fprintf(fl, "* Is questing available at this time?\n"
  	      "quest = %d\n\n", CONFIG_QUEST_ACTIVE);
...
static void cedit_disp_game_play_options(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n\r\n"
        ...
        "%s4%s) Plague Contagious       : %s%s\r\n"
        "%s5%s) Quests Active           : %s%s\r\n"
        "%s6%s) Scripts on PC's         : %s%s\r\n"
        "%sQ%s) Exit To The Main Menu\r\n"
        "Enter your choice : ",
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.pk_allowed),
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.pt_allowed),
        grn, cyn, nrm, OLC_CONFIG(d)->play.level_can_shout,
        grn, cyn, nrm, OLC_CONFIG(d)->play.holler_move_cost,
        grn, cyn, nrm, OLC_CONFIG(d)->play.tunnel_size,
        grn, cyn, nrm, OLC_CONFIG(d)->play.max_exp_gain,
        grn, cyn, nrm, OLC_CONFIG(d)->play.max_exp_loss,
        grn, cyn, nrm, OLC_CONFIG(d)->play.max_npc_corpse_time,
        grn, cyn, nrm, OLC_CONFIG(d)->play.max_pc_corpse_time,

        grn, cyn, nrm, OLC_CONFIG(d)->play.idle_void,
        grn, cyn, nrm, OLC_CONFIG(d)->play.idle_rent_time,
        grn, cyn, nrm, OLC_CONFIG(d)->play.idle_max_level,
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.dts_are_dumps),
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.load_into_inventory),
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.track_through_doors),
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.disp_closed_doors),
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.no_mort_to_immort),

        grn, cyn, nrm, OLC_CONFIG(d)->play.OK,
        grn, cyn, nrm, OLC_CONFIG(d)->play.NOPERSON,
        grn, cyn, nrm, OLC_CONFIG(d)->play.NOEFFECT,
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.plague_is_contagious),
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.quest_active),
        grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->play.script_players),
        grn, nrm
        );
...
static void cedit_disp_crash_save_options(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n\r\n"
  	...
  	"%sQ%s) Exit To The Main Menu\r\n"
  	"Enter your choice : ",
  	grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->csd.free_rent),
  	grn, cyn, nrm, OLC_CONFIG(d)->csd.max_obj_save,
  	grn, cyn, nrm, OLC_CONFIG(d)->csd.min_rent_cost,
  	grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->csd.auto_save),
  	grn, cyn, nrm, OLC_CONFIG(d)->csd.autosave_time,
  	grn, cyn, nrm, OLC_CONFIG(d)->csd.crash_file_timeout,
  	grn, cyn, nrm, OLC_CONFIG(d)->csd.rent_file_timeout,
  	grn, nrm
  	);
...
static void cedit_disp_room_numbers(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n\r\n"
  	...
  	"%sQ%s) Exit To The Main Menu\r\n"
  	"Enter your choice : ",
  	grn, cyn, nrm, OLC_CONFIG(d)->room_nums.mortal_start_room,
  	grn, cyn, nrm, OLC_CONFIG(d)->room_nums.immort_start_room,
  	grn, cyn, nrm, OLC_CONFIG(d)->room_nums.frozen_start_room,
  	grn, cyn, nrm, OLC_CONFIG(d)->room_nums.donation_room_1,
  	grn, cyn, nrm, OLC_CONFIG(d)->room_nums.donation_room_2,
  	grn, cyn, nrm, OLC_CONFIG(d)->room_nums.donation_room_3,
  	grn, nrm
  	);
...
static void cedit_disp_operation_options(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n\r\n"
    ...
    "%sQ%s) Exit To The Main Menu\r\n"
    "Enter your choice : ",
    grn, cyn, nrm, OLC_CONFIG(d)->operation.DFLT_PORT,
    grn, cyn, nrm, OLC_CONFIG(d)->operation.DFLT_IP ? OLC_CONFIG(d)->operation.DFLT_IP : "<None>",
    grn, cyn, nrm, OLC_CONFIG(d)->operation.DFLT_DIR ? OLC_CONFIG(d)->operation.DFLT_DIR : "<None>",
    grn, cyn, nrm, OLC_CONFIG(d)->operation.LOGNAME ? OLC_CONFIG(d)->operation.LOGNAME : "<None>",
    grn, cyn, nrm, OLC_CONFIG(d)->operation.max_playing,
    grn, cyn, nrm, OLC_CONFIG(d)->operation.max_filesize,
    grn, cyn, nrm, OLC_CONFIG(d)->operation.max_bad_pws,
    grn, cyn, nrm, YESNO(OLC_CONFIG(d)->operation.siteok_everyone),
    grn, cyn, nrm, YESNO(OLC_CONFIG(d)->operation.nameserver_is_slow),
    grn, cyn, nrm, YESNO(OLC_CONFIG(d)->operation.use_new_socials),
    grn, cyn, nrm, YESNO(OLC_CONFIG(d)->operation.auto_save_olc),
    grn, cyn, nrm, OLC_CONFIG(d)->operation.MENU ? OLC_CONFIG(d)->operation.MENU : "<None>",
    grn, cyn, nrm, OLC_CONFIG(d)->operation.WELC_MESSG ? OLC_CONFIG(d)->operation.WELC_MESSG : "<None>",
    grn, cyn, nrm, OLC_CONFIG(d)->operation.START_MESSG ? OLC_CONFIG(d)->operation.START_MESSG : "<None>",
    grn, nrm
    );
...
static void cedit_disp_autowiz_options(struct descriptor_data *d)
{
  ...
  write_to_output(d, "\r\n\r\n"
    ...
    "%sQ%s) Exit To The Main Menu\r\n"
    "Enter your choice : ",
    grn, cyn, nrm, CHECK_VAR(OLC_CONFIG(d)->autowiz.use_autowiz),
    grn, cyn, nrm, OLC_CONFIG(d)->autowiz.min_wizlist_lev,
    grn, nrm
    );
  ...
  switch (OLC_MODE(d)) {
    ...
    case CEDIT_GAME_OPTIONS_MENU:
	  ...
	  TOGGLE_VAR(OLC_CONFIG(d)->play.no_mort_to_immort);
        ...
        case '4':
          TOGGLE_VAR(OLC_CONFIG(d)->play.plague_is_contagious);
          break;

        case '5':
          TOGGLE_VAR(OLC_CONFIG(d)->play.quest_active);
          break;
