...
ACMD(do_oasis_hedit)
{
  ...
  if (!can_edit_zone(ch, HEDIT_PERMISSION)) {
    sendChar(ch, "You don't have access to editing help files.\r\n");
    return;
  }

  for (d = descriptor_list; d; d = d->next) {
    if (STATE(d) == CON_HEDIT) {
      sendChar(ch, "Sorry, only one can person can edit help files at a time.\r\n");
      return;
    }
  }
  ...
  if (!*arg) {
    sendChar(ch, "Please specify a help entry to edit.\r\n");
    return;
  }
  ...
  if (OLC_ZNUM(d) == NOWHERE) {
    sendChar(ch, "Do you wish to add the '%s' help file? ", OLC_STORAGE(d));
    OLC_MODE(d) = HEDIT_CONFIRM_ADD;
  } else {
    sendChar(ch, "Do you wish to edit the '%s' help file? ", help_table[OLC_ZNUM(d)].keywords);
...
static void hedit_save_to_disk(struct descriptor_data *d)
{
  ...
  if (!(fp = fopen(index_name, "w"))) {
    mlog("SYSERR: Could not write help index file");
    return;
  }
...
static void hedit_disp_menu(struct descriptor_data *d)
{
  ...
  write_to_output(d,
      ...
      "Enter choice : ",
       nrm,
       cyn, grn, nrm, OLC_HELP(d)->entry,
       cyn, grn, nrm, OLC_HELP(d)->min_level,
       cyn, nrm
  );
...
void hedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case HEDIT_MAIN_MENU:
    ...
    case '1':
      ...
      clear_screen(d);
      write_to_output(d, "Enter helpfile description: (/s saves /h for help)\r\n");
      SEND_RULER(d);
      if (OLC_HELP(d)->entry) {
...
ACMD(do_helpcheck)
{
  sendChar(ch, "Work still in progress for this command:\r\n");
  return;
}
...
ACMD(do_hindex)
{
  ...
  if (!*argument) {
    sendChar(ch, "Usage: hindex <string>\r\n");
    return;
  }
