...
OCMD(do_oecho)
{
	...
	// This was quite noisy and didn't seem to cause any problems.
...
OCMD(do_otimer)
{
  ...
  if (!*arg)
    ...
    int time = atoi(arg);
    if (time) SET_BIT_AR(obj->obj_flags.extra_flags, ITEM_TIMED);
    else REMOVE_BIT_AR(obj->obj_flags.extra_flags, ITEM_TIMED);
...
OCMD(do_odamage) {
	...
	if (GET_POS(ch) == POS_DEAD) {
	    if (!IS_NPC(ch)) {
		mudlog(BRF, LVL_HERO, TRUE, "%s killed by a trap at %s", GET_NAME(ch),
