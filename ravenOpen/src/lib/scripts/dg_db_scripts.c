...
void dg_read_trigger(FILE *fp, void *proto, int type)
{
  ...
  if (count != 2) {
    ...
    mlog("TRIGERR: Error assigning trigger!");
    return;
  }

  rnum = real_trigger(vnum);
  if (rnum<0) {
    mlog("TRIGERR: Trigger vnum #%d asked for but non-existant!", vnum);
    return;
  }
  ...
  switch(type) {
    ...
    case WLD_TRIGGER:
      ...
      if (rnum>=0) {
        ...
        mlog("TRIGERR: non-existant trigger #%d assigned to room #%d",
          vnum, room->number);
      }
      break;
    default:
      mlog("TRIGERR: Trigger vnum #%d assigned to non-mob/obj/room",
              vnum);
  }
}
  ...
  switch (type)
  {
    case MOB_TRIGGER:
      ...
      while (trg_proto) {
        ...
        if (rnum==-1) {
          mlog("TRIGERR: trigger #%d non-existant, for mob #%d",
            trg_proto->vnum, mob_index[mob->nr].number);
        } else {
          if (!SCRIPT(mob))
    ...
    case OBJ_TRIGGER:
      ...
      while (trg_proto) {
        rnum = real_trigger(trg_proto->vnum);
        if (rnum==-1) {
          mlog("TRIGERR: trigger #%d non-existant, for obj #%d",
            trg_proto->vnum, obj_index[obj->item_number].number);
        } else {
          if (!SCRIPT(obj))
    ...
    case WLD_TRIGGER:
      ...
      while (trg_proto) {
        rnum = real_trigger(trg_proto->vnum);
        if (rnum==-1) {
          mlog("TRIGERR: trigger #%d non-existant, for room #%d",
      ...
      break;
    default:
      mlog("TRIGERR: unknown type for assign_triggers()");
      break;
  }
}
