...
/** Static function protos (local file lock) */
static void find_replacement(void *go, struct script_data *sc, trig_data *trig,
                int type, char *var, char *field, char *subfield, char *str, size_t slen);
static void var_subst(void *go, struct script_data *sc, trig_data *trig,
...
int find_eq_pos_script(struct char_data * ch, struct obj_data * obj, char *arg)
{
  if (strcmp(arg, "hold") == 0) return WEAR_HOLD;
  if (strcmp(arg, "light") == 0) return WEAR_LIGHT;
  if (strcmp(arg, "wield") == 0) return WEAR_WIELD;
  return find_eq_pos(ch, obj, arg);
}
...
void do_stat_trigger(struct char_data *ch, trig_data *trig)
{
    ...
    if (!trig)
    {
	mlog("TRIGERR: NULL trigger passed to do_stat_trigger.");
	return;
    }
...
{  
  ...
  if (!str_cmp(arg1, "room")) {
    ...
    if (!SCRIPT(room))
      sendChar(ch, "This room does not have any triggers.\r\n");
    else if (!str_cmp(arg2, "all")) {
      extract_script(room, WLD_TRIGGER);
      sendChar(ch, "All triggers removed from room.\r\n");
    } else if (remove_trigger(SCRIPT(room), arg2)) {
      sendChar(ch, "Trigger removed.\r\n");
      if (!TRIGGERS(SCRIPT(room))) {
        extract_script(room, WLD_TRIGGER);
      }
    } else
      sendChar(ch, "That trigger was not found.\r\n");
  }
...
void script_log(char *msg)
{
  mudlog( NRM, LVL_GOD, TRUE, "SCRIPT ERR: %s", msg);
}
...
struct trig_var_data *find_var(trig_data *trig, char *var)
{
...
/** The slen parameter was added into function due to sprinttype being updated.
 *  It's only purpose is to guard against overflows from string parameter being
 *  passed from var_subst. Lenghty explanation due to neglected compile warning.
 *  -Xiuh 20090104 */
static void find_replacement(void *go, struct script_data *sc, trig_data *trig,
                int type, char *var, char *field, char *subfield, char *str, size_t slen)
{
  ...
  if (!*field) {
	...
	*str = '\0';
	...
	if (!str_cmp(field, "char")) {
	  ...
	  if (rndm)
	    ...
	    *str = '\0';
          ...
          struct trig_var_data *vd2=find_var(trig, field);
          
          if (vd2) {
            sprintf(str, "%d", ((num = atoi(vd2->value))>0) ? number(1,num):0);
          } else
	...
	sprintf(str, "%d", GET_ALIGNMENT(c));
      ...
      {
        if (PRF_FLAGGED(c, PRF_GOLD_TEAM ))
           strcpy(str, "gold");
        else if (PRF_FLAGGED(c, PRF_BLACK_TEAM ))
           strcpy(str, "black");
        else if (PRF_FLAGGED(c, PRF_ROGUE_TEAM ))
           strcpy(str, "rogue");
      }
        ...
        if (subfield && *subfield) {
          ...
          GET_GOLD(c) += addition;
          mudlog(BRF, LVL_IMMORT, TRUE, "%s has received %d gold from trigger %d.",
                    GET_NAME(c), addition, GET_TRIG_VNUM(trig));
        }
        ...
        if (subfield && *subfield) {
          ...
          GET_EXP(c) += addition;
          mudlog(BRF, LVL_IMMORT, TRUE, "%s has received %d experience points from trigger %d.",
                    GET_NAME(c), addition, GET_TRIG_VNUM(trig));
        }
        sprintf(str, "%d", GET_EXP(c));
      }

      else if (!str_cmp(field, "qp")) {
        if (subfield && *subfield) {
          int addition = MAX(MIN(atoi(subfield), 5), -5);
          GET_QP(c) += addition;
          mudlog(BRF, LVL_IMMORT, TRUE, "%s has received %d quest points from trigger %d.",
                    GET_NAME(c), addition, GET_TRIG_VNUM(trig));
        }
        sprintf(str, "%d", GET_QP(c));
      }
	...
	if (IS_NPC(c))
      ...
      else if (!str_cmp(field, "is_grouped")) {
	...
	sprintf(str, "%d", world[IN_ROOM(c)].number);
        ...
        sprintf(str, "%d", GET_QUEST(c) ? GET_QUEST(c)->virtual : 0);

      else if (!str_cmp(field, "canquest")) {
        int quest = real_quest(atoi(subfield));
        sprintf(str, "%d",
            quest == -1 ? 0 : can_take_quest(c, qst_list + quest, NULL));
      }

      else if (!str_cmp(field, "path")) {
        sh_int destvnum = atoi(subfield);
        int dir;
        char logbuf[500];
        dir = find_first_step(real_room(world[IN_ROOM(c)].number),
            real_room(destvnum));
        switch (dir) {
          case BFS_ERROR:
          case BFS_NO_PATH: strcpy(str, "!"); break;
          case BFS_ALREADY_THERE: strcpy(str, "@"); break;
          default: strcpy(str, dirs[dir]); break;
        }
      }

      else if (!str_cmp(field, "eq")) {
        int pos = find_eq_pos_script(c, NULL, subfield);
        ...
        FollowType *f;
        f = c->followers;
        while (f && pos) f = f->next, pos--;
        if (f)
          sprintf(str, "%c%ld", UID_CHAR, GET_ID(f->follower));
        else strcpy(str, "");
      }
	...
	*str = '\0';
	...
	if (r->dir_option[DOWN])
	  ...
	  *str = '\0';
        ...
        sh_int destvnum = atoi(subfield);
        int dir = find_first_step(r->number, destvnum);
        switch (dir) {
          case BFS_ERROR:
          case BFS_NO_PATH: strcpy(str, "!"); break;
          case BFS_ALREADY_THERE: strcpy(str, "@"); break;
          default: sprintf(str, "%d", dir); break;
        }
      } else {
	*str = '\0';
	sprintf(buf2,
	...
	script_log(buf2);
      ...
      /* a string, do the text processing */
      if (text_processed(field, subfield, vd, str)) return;
      *str = '\0';
    }
  }
...
/* processes a script meta command */
void process_meta(void *go, struct script_data *sc, trig_data *trig,
		 int type, char *cmd)
{
  char arg[MAX_INPUT_LENGTH], name[MAX_INPUT_LENGTH];
  char intermediate[MAX_INPUT_LENGTH + 2];
  char result[MAX_INPUT_LENGTH], *expr;
  ...
  eval_expr(expr, intermediate + 1, go, sc, trig, type);
  intermediate[0] = '%';
  intermediate[strlen(intermediate) + 1] = '\0';
  intermediate[strlen(intermediate)] = '%';
  eval_expr(intermediate, result, go, sc, trig, type);
  add_var(&GET_TRIG_VARS(trig), name, result, sc->context);
}
...
void process_qoffer(void *go, struct script_data *sc, trig_data *trig,
                    int type, char *cmd)
{
  char name[MAX_INPUT_LENGTH], target[MAX_INPUT_LENGTH], *arg;
  CharData *ch = (CharData *)go;

  arg = two_arguments(cmd, name, target);
  skip_spaces(&arg);

  if (!*arg) {
    sprintf(buf2, "Trigger: %s, VNum %d. qoffer w/o an arg: '%s'",
	    GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), cmd);
    script_log(buf2);
  } else if (type != MOB_TRIGGER) {
    sprintf(buf2, "Trigger: %s, VNum %d. qoffer only works for MOB scripts",
	    GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig));
    script_log(buf2);
  } else {
    CharData *t = get_char_room_vis(ch, target);
    int q = real_quest(atoi(arg));

    if (!t) {
      sprintf(buf2, "Trigger: %s, VNum %d. invalid target for qoffer: '%s'",
              GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), target);
      script_log(buf2);
    } else {
      if (can_take_quest(t, qst_list + q, NULL)) {
        give_quest(t, qst_list + q, ch);
      } else {
        sprintf(buf2, "Trigger: %s, VNum %d. Quest %d can't be given",
              GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), q);
        script_log(buf2);
      }
    }
  }
...
/* have to be tricky and remove stuns before handing off to the real
 * command interpreter, else our wonderful commands just get ignored!
 */
static void mob_command_interpreter(char_data *ch, char *cmd)
{
  int stun = ch->player.stunned;

  ch->player.stunned = 0;
  command_interpreter(ch, cmd);
  ch->player.stunned = MAX(stun, ch->player.stunned);
}
...
int script_driver(void *go, trig_data *trig, int type, int mode)
{
  ...
  for (cl = (mode == TRIG_NEW) ? trig->cmdlist : trig->curr_state;
	...
	cl = find_else_end(trig, cl, go, sc, type);
      ...
      if (cl->original && process_if(orig_cmd + 6, go, sc, trig,
        ...
        if (GET_TRIG_LOOPS(trig) == 100) {
          ...
          mudlog( NRM, LVL_GOD, TRUE,"SCRIPTERR: Trigger VNum %d has looped 100 times!!!",
            GET_TRIG_VNUM(trig));
          free(buf);
        }
      }
	...
	process_eval(go, sc, trig, type, cmd);
      ...
      else if (!strn_cmp(cmd, "meta ", 5))
	process_meta(go, sc, trig, type, cmd);
	...
	process_detach(go, sc, trig, type, cmd);
        ...
        mudlog(NRM, LVL_GOD, TRUE, DG_SCRIPT_VERSION);

      else if (!strn_cmp(cmd, "qoffer ", 7))
        process_qoffer(go, sc, trig, type, cmd);
