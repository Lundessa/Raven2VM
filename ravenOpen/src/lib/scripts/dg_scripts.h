...
struct trig_data {
    ...
    byte purged;			/* trigger is set to be purged     */
...
struct script_data {
  ...
  byte purged;				/* script is set to be purged */
...
// Mortius 110906: Changed functions below to ints as some calls are made
// using IF statements
// void entry_memory_mtrigger(struct char_data *ch);
// void greet_memory_mtrigger(struct char_data *ch);
int greet_memory_mtrigger(struct char_data *ch);
int entry_memory_mtrigger(struct char_data *ch);
