...
#define THIS_SKILL                SKILL_DO_DODGE

#define SKILL_ADVANCES_WITH_USE   TRUE
#define SKILL_ADVANCE_STRING      "You become on with the matrix"
...
#   define MIN_MOVE_COST     25
    ...
    if (IS_AFFECTED(ch, AFF_DO_DODGE)) {
        send_to_char("You'll now be much more elusive.\r\n", ch);
        affect_from_char(ch, THIS_SKILL);
        return;
    }

    if( GET_MOVE(ch) < dodge_cost ){
        send_to_char("You're too exhausted.\r\n", ch);
        return;
    }/* if */

    GET_MOVE(ch) -= dodge_cost;

    send_to_char("Okay, you'll try now be much more elusive.\r\n", ch);

    if( IS_AFFECTED(ch, AFF_DO_DODGE)) return;
