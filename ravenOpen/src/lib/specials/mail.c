...
void index_mail(long id_to_index, long pos, short type, time_t mail_time)
{
    ...
    switch (next_block.block_type) {
      ...
      case PARCEL_BLOCK:
        index_mail(next_block.header_data.to, block_num * BLOCK_SIZE,
                next_block.block_type, next_block.header_data.mail_time);
        total_messages++;
        break;
      case DELETED_BLOCK:
...
void store_mail(long to, long from, char *message_pointer)
{
  ...
  index_mail(to, target_address, HEADER_BLOCK, time(0));
...
static long read_delete_shared(long recipient)
{
...
ObjData *read_delete_object(long recipient, char *sender)
{
  long mail_address = read_delete_shared(recipient);
  header_block_type header;
  char *from_name = NULL;
  ObjData *obj;
  ObjFileElem store;
  ...
  if (header.block_type != PARCEL_BLOCK) {
    mlog("SYSERR: Oh dear.");
    no_mail = 1;
    mlog("SYSERR: Mail system disabled!  -- Error #9.");
    mlog("SYSERR: Mail to %s, block type %lu, mail address %lu",
            get_name_by_id(recipient), header.block_type, mail_address);
    return 0;
  }

  /* record who sent the package */
  from_name = get_name_by_id(header.header_data.from);
  if (!from_name) from_name = "";
  if (sender) strcpy(sender, from_name);

  memcpy(&store, header.txt, sizeof(store));
  obj = Obj_from_store(store);
			  ...
			  int cmd, char *arg)
{
  ...
  if(IS_HUNTED(ch)) {
      sprintf(buf, "$n tells you, 'Come back when you're not hunted.'");
      act(buf, FALSE, mailman, 0, ch, TO_VICT);
      return;
  }
  ...
  /* sending an object? */
  if (*buf2) {
      if ((obj = get_obj_in_list_vis(ch, buf2, ch->carrying)) == NULL) {
          act("$n tells you, 'What do you wish to send?'",
                  FALSE, mailman, 0, ch, TO_VICT);
          return;
      }
      if (Crash_is_unrentable(obj)) {
          act("$n tells you, 'I cannot send that for you.'",
                  FALSE, mailman, 0, ch, TO_VICT);
          return;
      }

      if (IS_CONTAINER(obj) && obj->contains) {
          act("$n tells you, 'You need to empty it first.'",
                  FALSE, mailman, 0, ch, TO_VICT);
          return;
      }

      if (IS_SET_AR(GET_OBJ_EXTRA(obj), ITEM_SOULBOUND) || contains_soulbound(obj)) {
          act("$n tells you, 'I am not allowed to handle that item.'",
                  FALSE, mailman, 0, ch, TO_VICT);
          return;
      }

      price = GET_OBJ_RENT(obj) * PARCEL_PRICE + 25000;
  }
  ...
  if (obj) {
    ...
    if (sizeof(ObjFileElem) >= HEADER_BLOCK_DATASIZE) {
        act("$n says, 'Notify an immortal, I'm broken.'",
                FALSE, mailman, obj, ch, TO_VICT);
        return;
    }
    header.block_type = PARCEL_BLOCK;
    header.header_data.next_block = LAST_BLOCK;
    header.header_data.from = GET_IDNUM(ch);
    header.header_data.to = recipient;
    header.header_data.mail_time = time(0);
    Obj_into_store(obj, (ObjFileElem *)header.txt);
    target_address = pop_free_list();
    index_mail(recipient, target_address, PARCEL_BLOCK, time(0));
    write_to_file(&header, BLOCK_SIZE, target_address);

    GET_GOLD(ch) -= price;
    sprintf(buf, "$n tells you, 'I'll take %d coins for the delivery.'", price);
    act(buf, FALSE, mailman, obj, ch, TO_VICT);
    act("$n takes $p from you with a promise to treat it carefully.",
            FALSE, mailman, obj, ch, TO_VICT);
    act("$N takes $p from $n and casually tosses it into a sorting bin.",
            FALSE, ch, obj, mailman, TO_ROOM);
    mlog("MAIL: %s mailed a %s (#%d) to %s",
            GET_NAME(ch), obj->short_description, GET_OBJ_VNUM(obj),
            get_name_by_id(recipient));
    extract_obj(obj);
    Crash_crashsave(ch);
  } else {
...
//128  SET_BIT(PLR_FLAGS(ch), PLR_MAILING | PLR_WRITING);
...
/* anonymously send the given object to the given recipient */
int postmaster_anonymous_mail(char *to, ObjData *obj)
{
    header_block_type header;
    long target_address;
    long recipient = get_id_by_name(to);

    if (recipient < 0) return 0;

    header.block_type = PARCEL_BLOCK;
    header.header_data.next_block = LAST_BLOCK;
    header.header_data.from = -1;
    header.header_data.to = recipient;
    header.header_data.mail_time = time(0);
    Obj_into_store(obj, (ObjFileElem *)header.txt);
    target_address = pop_free_list();
...
int mail_type(mail_index_type *mail)
{
    position_list_type *type = mail->list_start;

    while (type && type->next) type = type->next;
...
void postmaster_receive_mail(struct char_data * ch, struct char_data *mailman,
			  int cmd, char *arg)
{
  char buf[256], sender[MAX_NAME_LENGTH+1];
  struct obj_data *obj;
  mail_index_type *mail;
  int y;
  ...
  while ((mail = find_char_in_index(GET_IDNUM(ch))) != NULL) {
    if (mail_type(mail) == HEADER_BLOCK) {
      obj = create_obj();
...
//    GET_OBJ_WEAR(obj) = ITEM_WEAR_TAKE | ITEM_WEAR_HOLD;
      ...
      obj = read_delete_object(GET_IDNUM(ch), sender);
      sender[0] = toupper(sender[0]);

      if (!obj) {
          act("$n worriedly asks you to report a bug.", FALSE, mailman, 0, ch,
                  TO_VICT);
      } else {
        mlog("MAIL: %s received a %s (#%d) from %s.",
                GET_NAME(ch), obj->short_description, GET_OBJ_VNUM(obj),
                sender ? sender : "a NULL sender");
        obj_to_char(obj, ch);
        sprintf(buf, "$n says, 'You have a package from %s.'", sender);
        if (!*sender)
            sprintf(buf, "$n says, 'You have an anonymous package.'");
        act(buf, FALSE, mailman, obj, ch, TO_VICT);
        act("$n gives you $p.", FALSE, mailman, obj, ch, TO_VICT);
        act("$N gives $p to $n.", FALSE, ch, obj, mailman, TO_ROOM);
        Crash_crashsave(ch);
      }
    }
  }
...
{
    mail_index_type *index = mail_index;

    if (auction.ticks != AUC_NONE) return;

    while (index) {
        position_list_type *package = index->list_start;
        position_list_type *last = NULL;

        while (package) {
            if (package->type == PARCEL_BLOCK
             && package->mail_time + PARCEL_LIFE < time(0)) {
                ObjData *prize;

                /* need to fiddle the package to be the start of the
                 * position list */
                if (last) {
                    last->next = package->next;
                    package->next = index->list_start;
                    index->list_start = package;
                }

                prize = read_delete_object(index->recipient, NULL);
                if (prize) {
                    CharData *postmaster = read_mobile(18073, VIRTUAL);

                    if (postmaster) {
                        obj_to_char(prize, postmaster);
                        char_to_room(postmaster, 0);
                        do_auction(postmaster, "1.", 0, 0);
                        extract_char(postmaster);
                    } else {
                        extract_obj(prize);
                    }
                }
            ...
            last = package; package = package->next;
        }
        index = index->next;
    }
}
