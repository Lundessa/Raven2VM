...
//128  REMOVE_BIT(ROOM_FLAGS(rnum), ROOM_HOUSE_CRASH);
...
void House_boot(void)
{
  ...
  if (!(fl = fopen(HCONTROL_FILE, "rb"))) {
    mlog("House control file does not exist.");
    return;
  }
  while (!feof(fl) && num_of_houses < MAX_HOUSES) {
    ...
    /* Liam -- for some reason this is getting screwed. */
    if ( temp_house.num_of_guests > MAX_GUESTS )
      temp_house.num_of_guests = MAX_GUESTS;

    house_control[num_of_houses++] = temp_house;

//128    SET_BIT(ROOM_FLAGS(real_house), ROOM_HOUSE | ROOM_PRIVATE);
//128    SET_BIT(ROOM_FLAGS(real_atrium), ROOM_ATRIUM);
  ...
  for (i = 0; i < num_of_houses; i++) {
    ...
    if (house_control[i].num_of_guests) {
      ...
      for (j = 0; j < house_control[i].num_of_guests; j++) {
        /*
        ** Let's make certain we have a valid id othherwise
        ** make Digger the guest to flag the problem.
        */
        char *theGuest = get_name_by_id( house_control[i].guests[j] );
        if( theGuest == NULL )
            house_control[i].guests[j] = get_id_by_name( "Digger" );
...
void hcontrol_destroy_house(struct char_data * ch, char *arg)
{
  ...
  if ((real_atrium = real_room(house_control[i].atrium)) < 0)
    mlog("SYSERR: House had invalid atrium!");
  else
//128    REMOVE_BIT(ROOM_FLAGS(real_atrium), ROOM_ATRIUM);
    REMOVE_BIT_AR(ROOM_FLAGS(real_atrium), ROOM_ATRIUM);

  if ((real_house = real_room(house_control[i].vnum)) < 0)
    mlog("SYSERR: House had invalid vnum!");
  else {
//128    REMOVE_BIT(ROOM_FLAGS(real_house),
//128	       ROOM_HOUSE | ROOM_PRIVATE | ROOM_HOUSE_CRASH);
...
void hcontrol_pay_house(struct char_data * ch, char *arg)
{
  ...
  if (!*arg)
    ...
    mlog("Payment for house %s collected by %s.", arg, GET_NAME(ch));
...
//128  if (!IS_SET(ROOM_FLAGS(ch->in_room), ROOM_HOUSE))
  if (IS_SET_AR(ROOM_FLAGS(ch->in_room), ROOM_HOUSE))
    ...
    /* Liam : Brainchild move -- there is a maximum number of guest
     *        but there was no check to see if it was exceeded! 
     *        Added check Nov 16th, 1995. 
     */
    ...
    {
      send_to_char ("Your house is FULL!\r\n", ch);
    }
  }
}
...
//128      if (IS_SET(ROOM_FLAGS(real_house), ROOM_HOUSE_CRASH))
...
int House_can_enter(struct char_data * ch, sh_int house)
{
    ...
    if( GET_LEVEL(ch) >= LVL_LRGOD || (i = find_house(house)) < 0 ) return 1;
...
bootMortuary()
{
  mudlog(NRM, LVL_IMMORT, TRUE, "MORTUARY: Corpse managment." );
}
...
bootLockers()
{
  mudlog(NRM, LVL_IMMORT, TRUE, "LOCKERS: Player locker managment." );
}
